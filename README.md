...
*The file structure of the Freesurfer data directory where the surface data lives is assumed to be "freesurfer_output/subject_id/**surfaces**/subject_id". For any changes related to hardcoded filepaths, you can look at updating the function get_paths() in MIND_pipeline_2026.py (lines 32 through to 80)*
...
- **projectname**="PROJECTNAME" -> what your want project to be named, which will also be the name of your final MIND network project directory (i.e., MIND-<projectname>)
- **parcellation**="aparc" # "500_sym.aparc" "HCP.coarse.fsaverage.aparc" "HCP.fsaverage.aparc" -> the parcellation you are planning to use (some examples shown)
- **feature_list_to_use**="['FA', 'MD', 'ICVF', 'ISOVF', 'OD', 'CT', 'MC', 'Vol', 'SD', 'SA']" -> which features you want to compute your MIND one - N.B., these will also be the name of the output folder (i.e., if you are using ['FA', 'MD'] your final folder will be called FA_MD). It also means that if you use the same features but in a different order you will get a new folder!! (i.e., if you are using ['MD', 'FA'] your final folder will be called MD_FA).
- **space**="native" # "fsaverage6" -> the space in which you want to run your anaylsis (options shown)
- **micro**="micro" # "no-micro" -> whether the features you are using include microstructural data, using micro if yes or no-micro if not
- **resample**="no-resample" # "resample" -> whether you need to resample data (you'll only need to resample if you are using one feature), using resample if yes or no-resample if not
- **return_qc**="return_qc" # "no-return_qc" -> whether you want the qc files saved out, using return_qc if yes and no-return_qc if not
- **create_files**="create_files" # "no-create_files" -> whether you want to generate the .mgz files for micro and the fsaverage6 files (if you accidentally say yes and they already exists, that's fine they are skipped), using create_files if yes and no-create_files if not
- **test_mode**="no-test_mode" # test_mode -> as mentioned above, whether you want to generate everything in your scratch dir or save out to a main project path 
